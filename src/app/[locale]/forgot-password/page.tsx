    ...
    <div className="page-section py-14 md:py-20">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.9fr_1.1fr]">
        <section className="space-y-6 rounded-[2.75rem] border border-[rgba(111,89,64,0.12)] bg-white/80 p-8 shadow-[0_18px_48px_rgba(24,21,17,0.06)] md:p-10">
          <Badge>{copy.navigation.forgotPassword}</Badge>
          ...
          <LocaleLink href="/sign-in" locale={typedLocale} className="text-sm text-[var(--color-ink)] underline underline-offset-4">
            {copy.navigation.signIn}
          </LocaleLink>
        </section>
        <AuthForm mode="forgot-password" copy={copy.auth} />
      </div>
    </div>
  );
}
